...
export default function ToolsPage(){
  ...
  return (
    <div>
      <div className="panel-header"><h2>Tools</h2><p>Browse tools with filters</p></div>
      <form className="filter-form" onSubmit={submit}>
        ...
        <label>Limit<input type="number" min={1} max={200} value={limit} onChange={(e)=>setLimit(Number(e.target.value))} /></label>
        <button type="submit">Fetch</button>
      </form>

      {error && <div className="error-message">{error}</div>}
      <div className="result-grid">
        {loading ? <p className="empty-state">Loading…</p> : tools.map(t=> (
